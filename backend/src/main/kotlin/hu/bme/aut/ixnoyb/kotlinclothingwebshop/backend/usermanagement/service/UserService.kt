...
import hu.bme.aut.ixnoyb.kotlinclothingwebshop.backend.usermanagement.service.UserService.Companion.ERROR_MESSAGE_UNEXPECTED_ERROR
...
class DefaultUserService(private val datasource: UserDatasource) : UserService {
    ...
    override suspend fun updateUser(updatableUserCandidate: UpdatableUserCandidate): User =
        try {
            ...
            when (e.message) {
                UserDatasource.ERROR_MESSAGE_USER_NOT_FOUND -> {
                    logger.error("User not found for update with ID: " +
                ...
                UserDatasource.ERROR_MESSAGE_USER_INVALID_PASSWORD -> {
                    logger.info(
                        "Invalid current password was given for user update operation " +
                                "for user ID: ${updatableUserCandidate.id.value}"
                    )
    ...
    override suspend fun deleteUser(password: Password, userID: UserID) {
        try {
            datasource.deleteUser(password, userID)
        } catch (e: IllegalArgumentException) {
            if (e.message == UserDatasource.ERROR_MESSAGE_USER_INVALID_PASSWORD) {
                logger.info(
                    "Invalid password was given for user deletion operation " +
                            "for user ID: ${userID.value}"
                )
